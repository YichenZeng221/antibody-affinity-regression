"""Training loop for the first residue-level interaction affinity baseline."""
...
from src.affinity_interaction_evaluate import (
    evaluate_interaction_affinity_model,
    interaction_model_inputs,
)
...
def print_dataset_filter_summary(split_name: str, dataset: InteractionAffinityDataset) -> None:
    """Show annotated rows kept after successful CDR extraction filtering."""

    print(
        f"{split_name} interaction rows kept: {len(dataset)} / {dataset.raw_row_count} "
        f"(filtered CDR extraction failures: {dataset.filtered_out_count})"
    )


def train_interaction_affinity(config: dict) -> Path:
    """Train HCDR3/LCDR3-antigen interaction model from one YAML config."""
    ...
    train_dataset = InteractionAffinityDataset(
    ...
    val_dataset = InteractionAffinityDataset(
    ...
    print("Interaction input mode: HCDR3 + LCDR3 + antigen_sequence")
    ...
    for epoch in range(int(config["epochs"])):
        model.train()
        total_loss = 0.0
        progress_bar = tqdm(train_dataloader, desc=f"Interaction affinity epoch {epoch + 1}")
        ...
        val_metrics, _, _ = evaluate_interaction_affinity_model(model, val_dataloader, device)
    ...
    print(f"Saved interaction-aware affinity checkpoint to {checkpoint_path}")
